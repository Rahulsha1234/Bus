        ...
        if ($agent_user_id > 0) {
            ...
            if (!empty($new_status)) {
                try {
                    ...
                    if ($stmt->rowCount() > 0) {
                        $success = "Agent status updated to " . strtoupper($new_status) . " successfully!";
                        log_activity($pdo, $_SESSION['user_id'], 'AGENT_STATUS_CHANGE', "Updated status of agent user ID $agent_user_id to $new_status");
                    } else {
                        $error = "Failed to update agent status. Verify agent user exists.";
                    }
                } catch (PDOException $e) {
                    $error = "Database error during write: " . $e->getMessage();
                }
            }
        }
    }
                                ...
                                <div class="d-flex justify-content-end gap-2">
                                    ...
                                    <?php elseif ($agent['status'] === 'approved'): ?>
                                        <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="POST" class="d-inline">
                                            ...
                                            <input type="hidden" name="agent_id" value="<?= $agent['user_id'] ?>">
                                            <button type="submit" class="btn btn-danger py-1 px-3 small" style="font-size:0.75rem;" onclick="return confirm('Suspend operations for this agency? All agent panels will be locked.')"><i class="fa-solid fa-user-slash me-1"></i>Suspend</button>
                                        </form>
                                    <?php else: ?>
                                        <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="POST" class="d-inline">
                                            <input type="hidden" name="csrf_token" value="<?= get_csrf_token() ?>">
