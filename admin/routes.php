    ...
    if (!verify_csrf_token($csrf_token)) {
        ...
        if ($action === 'add') {
            ...
            if (empty($source) || empty($destination) || $distance === 0 || empty($pickups) || empty($drops)) {
                ...
                try {
                    $stmt = $pdo->prepare("
                        INSERT INTO routes (agent_id, admin_id, source, destination, distance_km, duration, pickup_points, drop_points, status) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active')
                    ");
                    $stmt->execute([
                        $admin_id,
...
// Fetch Agent's Routes with Pagination
try {
    $count_stmt = $pdo->prepare("SELECT COUNT(*) FROM routes WHERE admin_id = ? AND status = 'active'");
    ...
    $stmt = $pdo->prepare("SELECT * FROM routes WHERE admin_id = ? AND status = 'active' ORDER BY id DESC LIMIT " . intval($limit) . " OFFSET " . intval($offset));
                            ...
                            <td class="text-end">
                                <div class="d-flex gap-2 justify-content-end">
                                    <button class="btn btn-secondary-glass py-1 px-2 edit-route-btn"
                                        data-id="<?= $route['id'] ?>" data-source="<?= htmlspecialchars($route['source']) ?>"
                                        ...
                                        data-bs-toggle="modal" data-bs-target="#editRouteModal"><i
                                            class="fa-solid fa-pen-to-square"></i></button>
                                    <button class="btn btn-secondary-glass py-1 px-2 text-danger small delete-route-btn"
                                        data-id="<?= $route['id'] ?>" data-bs-toggle="modal"
                                        data-bs-target="#deleteRouteModal"><i class="fa-solid fa-trash-can"></i></button>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
