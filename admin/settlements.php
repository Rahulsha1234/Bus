...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_csrf_token($csrf_token)) {
        ...
        if ($action === 'generate') {
            try {
                // Find all agents
                $agents_stmt = $pdo->query("SELECT id, username FROM users WHERE role = 'agent' AND status = 'approved'");
                $approved_agents = $agents_stmt->fetchAll();

                $generated_count = 0;
                $today_date = date('Y-m-d');

                foreach ($approved_agents as $ag) {
                    $agent_id = $ag['id'];

                    // Find the date range of uncaptured bookings for this agent
                    // We look at bookings that are 'paid' but not associated with any week cycle yet.
                    // To keep it simple and robust, we fetch the range of all paid bookings that have no settlement coverage.
                    $range_stmt = $pdo->prepare("
                        ...
                        WHERE bs.agent_id = :agent_id
                          ...
                          AND NOT EXISTS (
                              ...
                              WHERE ws.agent_id = :agent_id 
                                AND DATE(b.created_at) BETWEEN ws.week_start AND ws.week_end
                          )
                    ");
                    $range_stmt->execute([':agent_id' => $agent_id]);
                ...
                if ($generated_count > 0) {
                    $success = "Weekly settlements generated successfully for $generated_count travel agents!";
                    log_activity($pdo, $_SESSION['user_id'], 'SETTLEMENT_GENERATE', "Generated settlements for $generated_count agencies.");
                } else {
                    $error = "All agency bookings are already accounted for in existing settlement cycles.";
                }
...
try {
    $stmt = $pdo->query("
        ...
        JOIN agent_profiles ap ON ws.agent_id = ap.user_id
