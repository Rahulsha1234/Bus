...
try {
    // Fetch Operator Code
    $code_stmt = $pdo->prepare("SELECT operator_code FROM users WHERE id = ? LIMIT 1");
    $code_stmt->execute([$admin_id]);
    $operator_code = $code_stmt->fetchColumn();
    ...
    ");
    $stmt->execute([':admin_id' => $admin_id, ':today' => $today]);
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE bs.admin_id = :admin_id 
          ...
          AND b.payment_status = 'paid'
    ");
    $stmt->execute([':admin_id' => $admin_id, ':start_week' => $start_of_week]);
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE bs.admin_id = :admin_id 
          ...
          AND b.payment_status = 'paid'
    ");
    $stmt->execute([':admin_id' => $admin_id, ':start_month' => $start_of_month]);
    ...
    // Commission Paid through settlements (represented as agent_id in settlements table)
    $stmt = $pdo->prepare("
        SELECT COALESCE(SUM(commission_payable), 0) 
        FROM weekly_settlements 
        WHERE agent_id = :admin_id AND status = 'paid'
    ");
    $stmt->execute([':admin_id' => $admin_id]);
    ...
    ");
    $stmt->execute([':admin_id' => $admin_id, ':start_date' => $start_date, ':end_date' => $end_date]);
